...
export declare const buildBenchmarkSelectionKey: ({
  jobId,
  benchmarkId,
  benchmarkIndex,
...
export declare const COMPARE_CHILD_RUN_TYPE = 'Benchmark';
...
export declare const getCompareBenchmarkResultScore: (
  job: EvaluationJob,
  benchmarkId: string,
  benchmarkIndex: number,
) => string;
export declare const getCompareRunEvaluationLabel: (
  job: EvaluationJob,
  collectionNameMap?: CollectionNameMap,
) => string;
export { formatDate as formatCompareTableDate } from '~/app/utilities/evaluationUtils';
export declare const getSelectionKeysCheckedState: (
  selectionKeys: string[],
  selectedBenchmarkKeys: Set<string>,
) => boolean | null;
export declare const getBenchmarkSelectionsFromKeys: (
  selectedBenchmarkKeys: Set<string>,
) => BenchmarkSelection[];
export declare const resolveComparableRunsFromSelections: (
  jobs: EvaluationJob[],
  selections: BenchmarkSelection[],
) => ComparableRunWithJobId[];
export declare const buildDefaultComparableRunsFromJobs: (
  jobs: EvaluationJob[],
) => ComparableRunWithJobId[];
export declare const buildMlflowCompareSearchParams: (
  runs: ComparableRunWithJobId[],
  resolveRunName: (run: ComparableRunWithJobId) => string,
) => string;
export declare const jobMatchesCompareBenchmarkSearch: (
  job: EvaluationJob,
  searchText: string,
) => boolean;
export declare const filterComparableRunsForCompareBenchmarkSearch: (
  job: EvaluationJob,
  searchText: string,
) => ComparableRun[];
export declare const filterJobsForCompareBenchmarkSearch: (
  jobs: EvaluationJob[],
  searchText: string,
) => EvaluationJob[];
