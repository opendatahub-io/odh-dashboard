...
describe('getRuntimeOdhPackages', () => {
  ...
  it('should include host @odh-dashboard/* dependencies as seeds', () => {
    const packages = [
      makePackage('odh-dashboard-frontend', {
        deps: {
    ...
    expect(result).toEqual(new Set(['@odh-dashboard/internal', '@odh-dashboard/plugin-core']));
  });
  ...
  it('should return empty set when no packages exist', () => {
    const result = getRuntimeOdhPackages([]);

    expect(result).toEqual(new Set());
  });

  it('should handle missing odh-dashboard-frontend in workspace', () => {
  ...
  it('should deduplicate packages reachable from multiple paths', () => {
    const packages = [
      ...
      makePackage('odh-dashboard-frontend', {
        deps: { '@odh-dashboard/plugin-core': '*' },
      }),
      makePackage('@odh-dashboard/plugin-core'),
    ];
