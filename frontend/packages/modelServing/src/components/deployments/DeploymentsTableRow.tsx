...
export const DeploymentRow: React.FC<{
  ...
  const [metricsExtension, metricsExtensionLoaded] = useResolvedDeploymentExtension(
    isModelServingMetricsExtension,
    deployment,
  );

  const metricsSupported =
    metricsExtensionLoaded && metricsExtension
      ? metricsExtension.properties.useIsMetricsSupported()
      : false;
    ...
    <ResourceTr resource={deployment.model}>
      <Td dataLabel="Name">
        <ResourceNameTooltip resource={deployment.model}>
          {metricsSupported && deployment.model.metadata.namespace ? (
            <Link
