...
const MarkdownComponent = ({
  ...
  const shiftHeadingLevel = (level: number): ContentVariants => {
    const adjusted = Math.min(level + maxHeading - 1, 6);
    // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
    return ContentVariants[`h${adjusted}` as keyof typeof ContentVariants];
  };
    ...
    <div className="markdown-content" data-testid={dataTestId}>
      <Markdown
        components={{
          ...
          h1: ({ children, ...props }) => (
            <Content component={shiftHeadingLevel(1)} {...props}>
              {children}
            </Content>
          ),
          h2: ({ children, ...props }) => (
            <Content component={shiftHeadingLevel(2)} {...props}>
              {children}
            </Content>
          ),
          h3: ({ children, ...props }) => (
            <Content component={shiftHeadingLevel(3)} {...props}>
              {children}
            </Content>
          ),
          h4: ({ children, ...props }) => (
            <Content component={shiftHeadingLevel(4)} {...props}>
              {children}
            </Content>
          ),
          h5: ({ children, ...props }) => (
            <Content component={shiftHeadingLevel(5)} {...props}>
              {children}
            </Content>
          ),
          h6: ({ children, ...props }) => (
            <Content component={shiftHeadingLevel(6)} {...props}>
              {children}
            </Content>
          ),
