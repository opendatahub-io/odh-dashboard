...
describe('Accelerator Profile', () => {
  ...
  it('list accelerator profiles', () => {
    ...
    tableToolbar.findFilterMenuOption('filter-dropdown-select', 'Name');
    tableToolbar.findSearchInput().fill('Test');
    acceleratorProfile.getRow('Test Accelerator').shouldHaveIdentifier('nvidia.com/gpu');

    tableToolbar.findFilterMenuOption('filter-dropdown-select', 'Identifier');
    tableToolbar.findResetButton().click();
    tableToolbar.findSearchInput().fill('tensor.com/gpu');
    tableRow.shouldHaveIdentifier('tensor.com/gpu');
  });
