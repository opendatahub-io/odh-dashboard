...
class TaskDrawer extends Contextual<HTMLElement> {
  ...
  findArgumentCodeBlock() {
    return this.findTaskDrawer().findByTestId('arguments-task-detail-code-block').findByRole('code');
  }
...
class PipelineDetails extends PipelinesTopology {
  ...
  getTaskDrawer() {
    return new TaskDrawer(() => cy.get('[data-testid="pipeline-task-name"]').parent());
  }
