...
describe(`Deploy NIM model`, () => {
  ...
  it(`Deploy ${models[0]} NIM and verify deployment`, () => {
    ...
    // Give some time for status tooltip to be visible
    cy.findByTestId('status-tooltip', { timeout: 60000 }).should('be.visible');
    modelServingSection.findStatusTooltip().click();
    // Wait for model tooltip status to be Loaded, fail early if Failed
    cy.findByTestId('model-status-tooltip', { timeout: 900000 })
    ...
    //     cy.log('Going to continue to loop until success because of BUG');
    //   }
      ...
      if (
        ...
        cy.log('Waiting for tooltip status to change to Loaded');
      }
    });
  });
});
