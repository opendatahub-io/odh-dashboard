...
describe('Verify user can deploy a model and access model metrics from UI', () => {
  retryableBefore(() => {
  ...
  it(
    ...
    () => {
      ...
      cy.step('Wait for metrics dashboard ConfigMap to be created (x validation)');
      checkMetricsDashboardConfigMap(modelName, projectName);

      cy.step('Verify metrics link exists and is accessible in UI');
      // Note reload is required as status tooltip was not found due to a stale element
      cy.reload();
      modelServingSection.findModelMetricsLink(modelName).should('exist');
      modelServingSection.findModelMetricsLink(modelName).should('not.be.disabled');
      modelServingSection.findModelMetricsLink(modelName).click();
      // Verify URL navigation and that the page loads
      cy.url().should('include', `/projects/${projectName}/metrics/model/${modelName}`);
      cy.step('✅ Model metrics access validation completed successfully');
      cy.log('Backend validation: ConfigMap exists and metrics are supported');
      cy.log('Frontend validation: Metrics link is accessible and page loads correctly');
    },
  );
});
