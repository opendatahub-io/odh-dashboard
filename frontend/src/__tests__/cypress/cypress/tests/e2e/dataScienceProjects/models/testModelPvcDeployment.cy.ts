...
describe('Verify a model can be deployed from a PVC', () => {
  ...
  it(
    ...
    () => {
      ...
      // TODO: Once deploy modal update merges, change this to PVC radio and path input
      inferenceServiceModal.findNewConnectionOption().click().scrollIntoView();
      inferenceServiceModal.findConnectionTypeDropdown().click();
      inferenceServiceModal.findUriNewConnectionType().click();
      inferenceServiceModal.findConnectionNameInput().type('Temp-Connection');
      inferenceServiceModal
        .findUriLocationPathInput()
        .type(`pvc://${pvStorageName}/${modelFilePath}`);
