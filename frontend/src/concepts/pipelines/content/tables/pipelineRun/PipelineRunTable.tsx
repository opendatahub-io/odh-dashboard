...
const PipelineRunTable: React.FC<PipelineRunTableProps> = ({
    ...
    isExperimentsAvailable && experimentId ? (
      <Tooltip content="Select up to 10 runs to compare">
        <Button
          ...
          variant="secondary"
          disabled={selectedIds.length === 0 || selectedIds.length > 10}
          onClick={() =>
  ...
  const actions = [primaryToolbarAction, toolbarDropdownAction];
  if (compareRunsAction) {
    actions.push(compareRunsAction);
  }
