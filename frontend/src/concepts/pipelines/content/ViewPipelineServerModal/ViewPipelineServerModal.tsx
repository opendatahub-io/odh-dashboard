...
const ViewPipelineServerModal: React.FC<ViewPipelineServerModalProps> = ({ isOpen, onClose }) => {
  const { pipelineNamespaceCR, namespace } = usePipelinesAPI();
  const { data: dataConnections } = useContextResourceData<DataConnection>(
    useDataConnections(namespace),
  );
  const [result] = useNamespaceSecret(namespace, 'pipelines-db-password');
              ...
              !!databaseSecret['db-password'] && (
                <>
                  ...
                  <DescriptionListGroup>
                    ...
                    <DescriptionListDescription>
                      <PasswordHiddenText password={databaseSecret['db-password']} />
                    </DescriptionListDescription>
                  </DescriptionListGroup>
                  <DescriptionListGroup>
