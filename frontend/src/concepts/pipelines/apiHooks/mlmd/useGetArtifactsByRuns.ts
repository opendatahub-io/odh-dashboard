...
export const useGetArtifactsByRunsForExperiment = (
  runs: PipelineRunKF[],
  experimentId: string,
): FetchState<Record<string, Artifact[]>[]> => {
  const { metadataStoreServiceClient } = usePipelinesAPI();
  ...
  const call = React.useCallback<FetchStateCallbackPromise<Record<string, Artifact[]>[]>>(
    () =>
      Promise.all(
        runs.map((run) => {
          ...
          return getMlmdContext(metadataStoreServiceClient, run.run_id, MlmdContextTypes.RUN).then(
            async (context) => {
              if (!context) {
                throw new Error(`No context for run: ${run.run_id}`);
              }
    ...
    [experimentId, metadataStoreServiceClient, runs],
  );
