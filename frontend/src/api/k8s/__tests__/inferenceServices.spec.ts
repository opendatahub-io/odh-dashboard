...
describe('assembleInferenceService', () => {
  ...
  it('should set accelerator profile annotation for accelerator profiles', () => {
    const acceleratorProfile = mockAcceleratorProfile({
    ...
    expect(result.metadata.annotations?.['opendatahub.io/accelerator-name']).toBe(
      'gpu-accelerator',
    );
    expect(result.metadata.annotations?.['opendatahub.io/hardware-profile-name']).toBeUndefined();
  });
  ...
  it('should not set hardware profile annotations when accelerator profile name is undefined', () => {
    ...
    expect(result.metadata.annotations?.['opendatahub.io/accelerator-name']).toBe('migrated-gpu');
    expect(result.metadata.annotations?.['opendatahub.io/hardware-profile-name']).toBeUndefined();
  });
