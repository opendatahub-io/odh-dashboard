...
describe('getSupportedAccessModesForProvisioner', () => {
  ...
  it('should return RWX support for ODF CephFS provisioner', () => {
    const modes = getSupportedAccessModesForProvisioner(StorageProvisioner.ODF_CEPHFS_CSI);
    expect(modes).toContain(AccessMode.RWX);
    expect(modes).toEqual(provisionerAccessModes[StorageProvisioner.CEPHFS_CSI]);
  });

  it('should return correct access modes for ODF RBD provisioner', () => {
    const modes = getSupportedAccessModesForProvisioner(StorageProvisioner.ODF_RBD_CSI);
    expect(modes).toEqual(provisionerAccessModes[StorageProvisioner.RBD_CSI]);
  });
