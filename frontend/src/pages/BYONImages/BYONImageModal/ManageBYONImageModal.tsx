...
import { createImageStream, updateImageStream } from '#~/api/k8s/imageStreams.ts';
...
const ManageBYONImageModal: React.FC<ManageBYONImageModalProps> = ({ existingImage, onClose }) => {
  ...
  const handleResponse = (promise: Promise<unknown>) => {
    setIsProgress(true);
    promise
      .then(() => {
        setIsProgress(false);
  ...
  const submit = () => {
    ...
    if (existingImage) {
      handleResponse(
        updateImageStream(dashboardNamespace, {
      ...
      handleResponse(
        createImageStream(dashboardNamespace, {
          // eslint-disable-next-line camelcase
          display_name: byonNameDesc.name,
