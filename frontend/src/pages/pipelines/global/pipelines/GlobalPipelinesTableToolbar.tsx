...
import { ToolbarChip } from '@patternfly/react-core/src/components/Toolbar/ToolbarFilter';
...
const GlobalPipelinesTableToolbar: React.FC<GlobalPipelinesTableToolbarProps> = ({
  ...
  if (filterData[FilterType.PIPELINE_NAME]) {
    chips.push({
      ...
      node: <>Name: {filterData[FilterType.PIPELINE_NAME]}</>,
    });
  }
  if (filterData[FilterType.CREATED_ON]) {
    chips.push({
      key: FilterType.CREATED_ON,
      node: <>Created: {filterData[FilterType.CREATED_ON]}</>,
    });
  }
