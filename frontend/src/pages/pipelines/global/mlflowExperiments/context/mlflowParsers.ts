...
const parseRegisteredModels = createEntityParser({
  type: 'model',
  objectKeys: ['registered_model'],
  arrayKeys: ['registered_models'],
  extract: (m) => {
...
const parseModelVersions = createEntityParser({
  type: 'version',
  objectKeys: ['model_version'],
  arrayKeys: ['model_versions'],
  extract: (v) => {
    const version = getString(v, 'version');
    const modelName = getString(v, 'name');
    return version && modelName
      ? { id: `${modelName}:${version}`, name: `Version ${version}` }
      : undefined;
  },
});
...
export const parseEntityNamesFromResponse = async (
  ...
  try {
    ...
    if (url.includes('/registered-models')) {
      parseRegisteredModels(data, setName);
    }

    if (url.includes('/model-versions')) {
      parseRegisteredModels(data, setName);
      parseModelVersions(data, setName);
    }
