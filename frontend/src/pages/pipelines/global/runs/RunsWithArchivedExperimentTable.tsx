...
import { ExperimentStatus, PipelineResourceRestoreResult } from './types';
...
type RunsWithArchivedExperimentProps = {
  ...
  runStatus: { run: PipelineRunKF; status: PipelineResourceRestoreResult }[];
              ...
              <Stack hasGutter>
                {run.map((singleRun, index) => {
                  const matchingRun = runStatus.find(
                    (currentRun) => currentRun.run.run_id === singleRun.run_id,
                  );
