...
type AddExistingStorageFormFieldProps = {
  data?: StorageData['creating'];
  setData: (data: StorageData['creating']) => void;
  selectDirection?: 'up' | 'down';
  menuAppendTo?: HTMLElement;
};

const AddExistingStorageFormField: React.FC<AddExistingStorageFormFieldProps> = ({
  data,
  setData,
  selectDirection,
  menuAppendTo,
        ...
        onSelect={(_ev, storage) =>
          setData({
            ...
            nameDesc: {
              name: String(storage),
              description:
                storages.find((s) => s.metadata.name === String(storage))?.metadata.annotations?.[
...
export default AddExistingStorageFormField;
