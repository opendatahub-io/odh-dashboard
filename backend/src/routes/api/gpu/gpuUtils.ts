...
import { V1Pod, V1PodList, V1Secret, V1Service, V1ServiceAccount } from "@kubernetes/client-node";
    ...
    const gpuPodList = await fastify.kube.coreV1Api
        ...
        if (gpuPodList.items.length != 0) {
            ...
            const promSA = await fastify.kube.coreV1Api
                ...
                .then((res) => {
                    return res.body as V1ServiceAccount;
                });
            let promTokenName = ""
            ...
            for (let i=0; i < gpuPodList.items.length; i++) {
                ...
                const request = http.get(options, callback).end();
