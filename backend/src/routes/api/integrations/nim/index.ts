    ...
    fastify.get(
        ...
        secureAdminRoute(fastify)(
            async (
                ...
                await getNIMAccount(fastify, namespace).then((response) => {
                    ...
                    if (e.response?.statusCode === 404) {
                        ...
                        if (isString(e.response.body) && e.response.body.trim() === PAGE_NOT_FOUND_MESSAGE.trim()) {
                            ...
                            reply.status(404).send({ isAppEnabled: false, canEnable: false, error: 'NIM not installed' });
                        } else {
    ...
    fastify.post(
        ...
        secureAdminRoute(fastify)(
            async (
                ...
                await createNIMSecret(fastify, namespace, enableValues).then(async () => {
                    await createNIMAccount(fastify, namespace).then((response) => {
                        if (isAppEnabled(response)) {
                            reply.send({ isAppEnabled: true, canEnable: false, error: '' });
                        } else {
                            reply.send({ isAppEnabled: false, canEnable: true, error: '' });
                        ...
                        reply.status(e.response.statusCode).send(new Error(`Failed to create NIM account, ${e.response?.body?.message}`));
                    })
                }).catch((e) => {
                    if (e.response?.statusCode === 409) {
                        ...
                        reply.status(e.response.statusCode).send(new Error(`Failed to create NIM secret, ${e.response?.body?.message}`));
                    }
                });
