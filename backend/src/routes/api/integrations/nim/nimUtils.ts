...
export const isAppEnabled = (app: NIMAccountKind): boolean => {
    ...
    return conditions.find((condition) => condition.type === 'AccountStatus' && condition.status === 'True') !== undefined;
}

export const getNIMAccount = async (fastify: KubeFastifyInstance, namespace: string): Promise<NIMAccountKind> => {
...
export const createNIMAccount = async (fastify: KubeFastifyInstance, namespace: string): Promise<NIMAccountKind> => {
    const { customObjectsApi } = fastify.kube;
    const account = {
...
export const createNIMSecret = async (fastify: KubeFastifyInstance, namespace: string, enableValues: { [key: string]: string }): Promise<void> => {
