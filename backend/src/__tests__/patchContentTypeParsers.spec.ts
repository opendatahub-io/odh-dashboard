...
describe('registerPatchContentTypeParsers', () => {
  ...
  it('should reject malformed json-patch payloads with 400', async () => {
    const app = fastify();
    registerPatchContentTypeParsers(app);
  ...
  it('should parse application/json PATCH with the built-in parser', async () => {
    const app = fastify();
    registerPatchContentTypeParsers(app);
    ...
    const response = await app.inject({
      ...
      payload: JSON.stringify({ stopped: true }),
    });
