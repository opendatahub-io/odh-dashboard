...
type tlsProfileSpec struct {
	...
	// Groups is the ordered list of TLS key-exchange groups (curves) allowed by
	// the profile. OpenShift exposes this on Custom profiles; the well-known
	// profiles leave it empty and rely on the component's default group set.
...
func ServerTLSConfig(ctx context.Context, logger *slog.Logger) (*tls.Config, error) {
	...
	if err != nil {
		...
		// In-cluster but the config could not be loaded (e.g. unreadable
		// token/CA file). Fail closed rather than silently weakening TLS.
		return nil, fmt.Errorf("failed to load in-cluster TLS configuration: %w", err)
	}
...
func serverTLSConfigFromREST(ctx context.Context, restCfg *rest.Config, logger *slog.Logger) (*tls.Config, error) {
	...
	// UnversionedRESTClientFor requires a NegotiatedSerializer. We read the
	// APIServer resource as raw JSON, so a bare codec factory (no registered
	// types) is sufficient — it avoids pulling in openshift/api schemes.
...
func tlsConfigFromProfile(profile *tlsSecurityProfile, logger *slog.Logger) (*tls.Config, error) {
	...
	// Enforce the profile's key-exchange groups (curves) when specified. The
	// well-known profiles leave Groups empty, so CurvePreferences stays nil and
	// Go's secure defaults apply. A Custom profile that restricts groups must be
	// honored, otherwise a client could negotiate a curve the admin excluded
	// (bypassing the cluster TLS policy).
	if len(spec.Groups) > 0 {
		curves, dropped := resolveGroups(spec.Groups)
		if len(dropped) > 0 {
			logger.Info("some groups from cluster TLS profile are not supported by Go, dropped",
				"dropped", dropped)
		}
		if len(curves) == 0 {
			return nil, fmt.Errorf("custom TLS profile specified %d groups but none are supported by Go", len(spec.Groups))
		}
	...
	logger.Info("TLS config derived from cluster profile",
		...
		"groupCount", len(tlsCfg.CurvePreferences))
...
// resolveGroups converts TLS group (curve) name strings to Go tls.CurveID values.
// Returns the resolved IDs and a list of names that could not be mapped.
func resolveGroups(names []string) ([]tls.CurveID, []string) {
