...
//
// On non-OpenShift clusters (IsNoMatchError), when the APIServer singleton
// does not exist (IsNotFound), or when the service account lacks RBAC
// permission to read it (IsForbidden), it returns Intermediate defaults
...
func serverTLSConfigFromREST(ctx context.Context, restCfg *rest.Config, logger *slog.Logger) (*tls.Config, error) {
	...
	if err != nil {
		if meta.IsNoMatchError(err) || errors.IsNotFound(err) || errors.IsForbidden(err) {
...
func reasonForError(err error) string {
	...
	if errors.IsForbidden(err) {
		return "forbidden: service account lacks permission to read APIServer resource"
	}
