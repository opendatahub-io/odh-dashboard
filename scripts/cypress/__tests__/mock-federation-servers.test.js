...
describe('listMockFederationServers', () => {
  it('includes host and federated packages with cypress coverage builds', () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(root, 'frontend', 'public-cypress', 'index.html'),
        '<html></html>',
      );
      fs.writeFileSync(
      ...
      fs.writeFileSync(
        path.join(pkgDir, 'frontend', 'public-cypress', 'remoteEntry.js'),
        '/* remote */',
      );
      fs.writeFileSync(
...
describe('assertWaitTargetsReady', () => {
  it('throws when remoteEntry.js is missing', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'odh-cy-'));
    try {
      fs.writeFileSync(
        path.join(root, 'pnpm-workspace.yaml'),
      ...
      const servers = listMockFederationServers(root);
      assert.throws(() => assertWaitTargetsReady(servers, root), /wait targets are missing/);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
