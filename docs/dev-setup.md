...
> Note: The CLI logged-in user will need to be a `cluster-admin` level user on the cluster to mimic the Dashboard Service Account level of permissions. You could also bind the [cluster role](../manifests/common/rbac/sa/cluster-role.yaml) to your user as we do with the service account [binding](../manifests/common/rbac/sa/cluster-role-binding.yaml).
