...
**RBAC:** Each module has least-privilege ClusterRole rules scoped to that BFF (see comments in parent [`modules-cluster-role.yaml`](../modules-cluster-role.yaml)). ClusterRoleBinding subjects receive a `namespace` when the module `kustomization.yaml` sets `namespace:` at build/apply time (operator or dev overlay).

**params.env:** Each module's [`params.env`](model-registry/params.env) holds only that module's image key(s). Values must stay in sync with [`../params.env`](../params.env); the release workflow updates them on version bump.
