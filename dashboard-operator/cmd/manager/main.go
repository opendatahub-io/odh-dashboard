...
func main() {
	...
	platform, err := cluster.DetectPlatform(context.Background(), mgr.GetClient(), os.Getenv("ODH_PLATFORM_TYPE"), operatorNamespace)
	if err != nil {
	...
	if err := mgr.AddReadyzCheck("api-server", apiServerReadyz(mgr.GetAPIReader())); err != nil {
