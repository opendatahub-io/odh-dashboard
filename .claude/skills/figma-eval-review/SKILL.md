...
|---|---|---|
| **Figma** (`plugin-figma-figma`) | Fetch design context, screenshot, and component metadata | Call `get_design_context` or `get_screenshot` with the provided Figma URL params |
...
3. If the call fails, stop and report:
   > Figma MCP is not available or not authenticated. This skill requires the Figma MCP (`plugin-figma-figma`) to fetch design context. Please install/authenticate the Figma plugin.
...
6. Attempt a lightweight Atlassian call (`jira_get_issue` with a dummy key format check) to verify the Atlassian MCP is reachable. Record whether it is available; do not fail if it is not.

If the required MCPs succeed, proceed. The screenshot from step 2 is reused in Phase 2 (no redundant fetch).

## Phase 2: Gather Context

### Step 1: Fetch Figma Design Context

Using the fileKey and nodeId extracted in Phase 1, fetch design data from the Figma MCP. Make these calls in parallel:
...
2. **`get_screenshot`** — returns a screenshot of the design node (already fetched in prerequisite check; reuse that result).
...
2. **Auto-detect from current branch** (default) — run `git branch --show-current` to get the branch name, then use the GitHub MCP's `search_pull_requests` (query: `repo:{owner}/{repo} head:{branch} is:open`) to find an open PR for that branch.
   ...
   - **If on a feature branch with no open PR:** fall back to `git diff main...HEAD` for local branch changes. Report that no PR was found.
...
Call `jira_get_issue` with the extracted key and `fields=summary,description,status,issuetype,labels,acceptance_criteria`. Extract:

- **Summary** — the one-line objective
- **Description** — full context including acceptance criteria, requirements, and any Figma URLs or node references embedded in the ticket
- **Acceptance criteria** — look for a section titled "Acceptance Criteria", "Requirements", or "Definition of Done"; extract each criterion as a discrete item
...
If the PR is in **draft** state, note this in the report header. Use `[SKIP]` rather than `[MISS]` for checks that appear not-yet-implemented.
...
| **Cannot Evaluate** | `[SKIP]` | Cannot determine conformance from static analysis alone (e.g., animation, responsive behavior, interaction timing) |
...
- **SKIP**: explain why static analysis is insufficient for this check
...
{Include the Figma screenshot fetched in Phase 2 for visual reference}
...
**Verdict:** {MATCH|PARTIAL|MISMATCH|SKIP}
...
- When a dimension has no applicable findings (e.g., no text changes for Content & Copy), use `[SKIP]` with a note explaining it was not applicable to this change set
