...
> **Recommended workflow**: Use `/module-onboarding` followed by `/konflux-onboarding` for the complete onboarding experience. These skills automate the majority of the manual steps described in this guide.
...
| Operator integration (dashboard-operator) | Module registry, proxy paths, image map, tests | RHOAIENG-37067 |

## What Platform Handles (External Repo)

| Step | Description | Related Subtask |
|------|-------------|-----------------|
| opendatahub-operator RELATED_IMAGE | Add `RELATED_IMAGE_ODH_MOD_ARCH_<UPPER_SNAKE>_IMAGE` to `opendatahub-io/opendatahub-operator` | RHOAIENG-37067 |

> **Note**: Tekton PipelineRun YAML files (`.tekton/` directory) are generated by the DevOps automation, not by the `/konflux-onboarding` skill.
