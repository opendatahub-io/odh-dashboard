...
- **Adding modules without updating the full chain** — new modules need entries in `modules.go` (registry with all fields), `modules_test.go` (count and name list), and `charts/dashboard/values.yaml` (related images). The `support.go` image mapping and proxy paths are read from the `ModuleDefinition` in the registry (single source of truth)
