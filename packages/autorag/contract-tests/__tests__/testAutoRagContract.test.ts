...
describe('AutoRAG API Contract Tests', () => {
  ...
  describe('MaaS Models Endpoint', () => {
    it('should retrieve all accessible MaaS models', async () => {
      const result = await apiClient.get(
        `/api/v1/maas/models?namespace=${NS}&secretName=maas-credentials`,
      );
      expect(result).toMatchContract(apiSchema, {
    ...
    it('should retrieve models with a Secret-backed MaaS configuration', async () => {
      const result = await apiClient.get(
        `/api/v1/maas/models?namespace=${NS}&secretName=maas-credentials`,
      );
      expect(result).toMatchContract(apiSchema, {
    ...
    it('should return 400 for an invalid secretName', async () => {
      const result = await apiClient.get(`/api/v1/maas/models?namespace=${NS}&secretName=INVALID`);
      expect(result.success).toBe(false);
      expect(result.error?.status).toBe(400);
    });
  });
  ...
  describe('S3 Files Endpoint', () => {
    describe('Error Cases - Missing Parameters', () => {
      ...
      it('should fall back to DSPA mode when secretName parameter is omitted', async () => {
      ...
      it('should return 400 when bucket parameter is missing and secret has no AWS_S3_BUCKET', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret`,
        );
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(400);
    ...
    describe('Error Cases - Empty Parameters', () => {
      ...
      it('should fall back to DSPA mode for empty secretName', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=&bucket=${BUCKET}`,
        );
        expect(result.success).toBe(true);
        if (result.success) {
          expect(result.response.status).toBe(200);
      ...
      it('should return 400 for empty bucket when secret has no AWS_S3_BUCKET', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret&bucket=`,
        );
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(400);
    ...
    describe('Error Cases - Secret Issues', () => {
      it('should return 404 when secret does not exist', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=non-existent-secret&bucket=my-bucket`,
        );
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(404);
      ...
      it('should return 404 when DSPA fallback fails in namespace without DSPA', async () => {
        const result = await apiClient.get(`/api/v1/s3/files?namespace=${NS_NO_DSPA}`);
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(404);
        }
    ...
    describe('Bucket Parameter Fallback', () => {
      it('should accept request without bucket query parameter when secret has AWS_S3_BUCKET', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket`,
        );
        expect(result).toMatchContract(apiSchema, {
      ...
      it('should allow bucket query parameter to override secret AWS_S3_BUCKET', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket&bucket=${BUCKET}`,
        );
        expect(result).toMatchContract(apiSchema, {
          ref: '#/components/responses/S3GetFilesResponse/content/application/json/schema',
          status: 200,
    ...
    describe('Search parameters handling', () => {
      const searchParamsUri = `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket`;
    ...
    describe('Success Cases', () => {
      ...
      it('should retrieve files with path parameter', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket&path=autorag%20input%20data`,
        );
        expect(result).toMatchContract(apiSchema, {
          ref: '#/components/responses/S3GetFilesResponse/content/application/json/schema',
          status: 200,
      ...
      it('should retrieve files with search parameter', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket&search=myfile`,
        );
        expect(result).toMatchContract(apiSchema, {
          ref: '#/components/responses/S3GetFilesResponse/content/application/json/schema',
          status: 200,
      ...
      it('should retrieve files with limit parameter', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket&limit=10`,
        );
        expect(result).toMatchContract(apiSchema, {
          ref: '#/components/responses/S3GetFilesResponse/content/application/json/schema',
          status: 200,
      ...
      it('should retrieve files with next (continuation token) parameter', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket&next=some-token`,
        );
        expect(result).toMatchContract(apiSchema, {
          ref: '#/components/responses/S3GetFilesResponse/content/application/json/schema',
          status: 200,
      ...
      it('should retrieve files with combined path, search, and limit parameters', async () => {
        const result = await apiClient.get(
          `/api/v1/s3/files?namespace=${NS}&secretName=test-secret-with-bucket&path=autorag%20input%20data&search=report&limit=5`,
        );
        expect(result).toMatchContract(apiSchema, {
          ref: '#/components/responses/S3GetFilesResponse/content/application/json/schema',
          status: 200,
  ...
  describe('Enable Managed Pipelines Endpoint', () => {
    describe('Success Cases', () => {
      it('should return 200 when enabling managed pipelines on a namespace with a DSPA', async () => {
        const result = await apiClient.post(
          `/api/v1/managed-pipelines/enable?namespace=${NS}`,
          undefined,
        );
        expect(result).toMatchContract(apiSchema, {
    ...
    describe('Error Cases', () => {
      ...
      it('should return 404 when no DSPA exists in namespace', async () => {
        const result = await apiClient.post(
          `/api/v1/managed-pipelines/enable?namespace=${NS_NO_DSPA}`,
          undefined,
        );
        expect(result.success).toBe(false);
        expect(result.error?.status).toBe(404);
    ...
    it('should create an indexing pipeline run with required fields', async () => {
      const result = await apiClient.post(`/api/v1/indexing-pipeline-runs?namespace=${NS}`, {
        ...
        parameters: {
          ...
          maas_secret_name: 'maas-credentials',
