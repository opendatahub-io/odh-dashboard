...
func (h *MaaSHandler) ModelsHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if !secretNameSupplied || len(secretNameValues) != 1 || secretNameValues[0] == "" {
		badRequestResponse(h.logger, w, r, "secretName is required")
		return
	}
	...
	result, err := h.service.ListModels(ctx, namespace, secretName)
	if err != nil {
		switch {
		...
		case errors.Is(err, repositories.ErrMaaSBadResponse):
			badGatewayResponseWithMessage(h.logger, w, r, err, "invalid response from hosted MaaS")
