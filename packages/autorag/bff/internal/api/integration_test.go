...
// This file contains API-level integration ("workflow") tests for the AutoRAG BFF.
//
// Unlike the OpenAPI contract tests (packages/autorag/contract-tests), which validate
...
// newIntegrationServer, rather than growing this file's existing test.
...
const integrationNamespace = "my-project"

// integrationHTTPClient is used for all requests issued by this suite. A bounded
...
// newIntegrationServer boots a real App wired entirely to stateful fakes (no live cluster,
...
// the fake identity extractor always returns a valid, fully-privileged identity, and
...
func newIntegrationServer(t *testing.T) string {
	...
	cfg := config.EnvConfig{
		...
		// names (packages/autorag/bff/internal/fake/pipelines.go) so pipeline-run
		// discovery succeeds — these mirror cmd/main.go's flag defaults.
		AutoRAGPipelineNamePrefix:  "documents-rag-optimization-pipeline",
		IndexingPipelineNamePrefix: "documents-indexing-pipeline",
	}
...
// integrationResponse captures a decoded HTTP response for assertions.
type integrationResponse struct {
	status int
	body   []byte
}

func integrationRequest(t *testing.T, method, url string, body any) integrationResponse {
	t.Helper()
	...
	return integrationResponse{status: resp.StatusCode, body: data}
}

func decodeInto(t *testing.T, resp integrationResponse, dst any) {
	t.Helper()
...
// TestPipelineRunLifecycle drives a single AutoRAG pipeline run through its full
...
func TestPipelineRunLifecycle(t *testing.T) {
	...
	runsURL := baseURL + "/api/v1/pipeline-runs?namespace=" + integrationNamespace

	var runID string

	t.Run("create returns a new PENDING run", func(t *testing.T) {
		resp := integrationRequest(t, http.MethodPost, runsURL, map[string]any{
			"display_name":           "integration-lifecycle-run",
		...
		if created.Data.DisplayName != "integration-lifecycle-run" {
			t.Errorf("display_name = %q, want %q", created.Data.DisplayName, "integration-lifecycle-run")
		}
	...
	t.Run("get reflects the created run's persisted state", func(t *testing.T) {
		...
		resp := integrationRequest(t, http.MethodGet, baseURL+"/api/v1/pipeline-runs/"+runID+"?namespace="+integrationNamespace, nil)
		if resp.status != http.StatusOK {
			t.Fatalf("expected 200, got %d: %s", resp.status, resp.body)
		}
		...
		if got.Data.DisplayName != "integration-lifecycle-run" {
			t.Errorf("display_name = %q, want %q", got.Data.DisplayName, "integration-lifecycle-run")
		}
	})
	...
	t.Run("list includes the created run", func(t *testing.T) {
		...
		resp := integrationRequest(t, http.MethodGet, runsURL, nil)
		if resp.status != http.StatusOK {
			t.Fatalf("expected 200, got %d: %s", resp.status, resp.body)
		}
	...
	t.Run("terminate transitions the run to a terminal FAILED state", func(t *testing.T) {
		...
		terminateResp := integrationRequest(t, http.MethodPost, baseURL+"/api/v1/pipeline-runs/"+runID+"/terminate?namespace="+integrationNamespace, map[string]any{})
	...
	t.Run("delete removes the run", func(t *testing.T) {
		...
		resp := integrationRequest(t, http.MethodDelete, baseURL+"/api/v1/pipeline-runs/"+runID+"?namespace="+integrationNamespace, nil)
		if resp.status != http.StatusOK {
...
func TestPipelineRunRetryToSuccess(t *testing.T) {
	...
	t.Run("create a run to retry", func(t *testing.T) {
		resp := integrationRequest(t, http.MethodPost, runsURL, map[string]any{
			"display_name":           "integration-retry-run",
			"test_data_secret_name":  "data-connection",
			"test_data_bucket_name":  "s3-bucket",
			"test_data_key":          "autorag input data/pdf/bank_policies_pdf/all_bank_policies_eval_data_pdf.json",
			"input_data_secret_name": "data-connection",
	...
	t.Run("retry resets the run and it progresses to SUCCEEDED", func(t *testing.T) {
		...
		retryResp := integrationRequest(t, http.MethodPost, baseURL+"/api/v1/pipeline-runs/"+runID+"/retry?namespace="+integrationNamespace, map[string]any{})
		...
		getResp := integrationRequest(t, http.MethodGet, baseURL+"/api/v1/pipeline-runs/"+runID+"?namespace="+integrationNamespace, nil)
...
// integrationUploadFile performs a multipart/form-data POST of content to the S3 upload
// endpoint using a "file" part named filename. The autorag S3 fake persists
...
func integrationUploadFile(t *testing.T, baseURL, key, filename, content string) integrationResponse {
	t.Helper()
	...
	uploadURL := baseURL + "/api/v1/s3/files/" + url.PathEscape(key) +
		"?namespace=" + integrationNamespace + "&secretName=data-connection&bucket=s3-bucket"
...
func TestS3FileUploadListDownloadRoundTrip(t *testing.T) {
	...
	key := fmt.Sprintf("integration-roundtrip-upload-%d.pdf", time.Now().UnixNano())
	content := "%PDF-1.4 integration roundtrip fixture content"

	t.Run("upload creates a new object", func(t *testing.T) {
		resp := integrationUploadFile(t, baseURL, key, key, content)
	...
	t.Run("list includes the uploaded file with the correct size", func(t *testing.T) {
		resp := integrationRequest(t, http.MethodGet,
			baseURL+"/api/v1/s3/files?namespace="+integrationNamespace+"&secretName=data-connection&bucket=s3-bucket", nil)
		if resp.status != http.StatusOK {
			t.Fatalf("expected 200, got %d: %s", resp.status, resp.body)
		}
	...
	t.Run("download returns the exact uploaded bytes", func(t *testing.T) {
		resp := integrationRequest(t, http.MethodGet,
			baseURL+"/api/v1/s3/files/"+url.PathEscape(key)+"?namespace="+integrationNamespace+"&secretName=data-connection&bucket=s3-bucket", nil)
		if resp.status != http.StatusOK {
			t.Fatalf("expected 200, got %d: %s", resp.status, resp.body)
		}
