...
jest.mock('~/app/components/common/MaasConnectionModal', () => ({
  ...
  default: ({
    ...
    <div data-testid="maas-connection-modal">
      ...
      <button data-testid="maas-modal-cancel" onClick={onClose}>
        Cancel
      </button>
    </div>
  ),
...
describe('AutoragCreate', () => {
  ...
  describe('Form validation', () => {
    it('should update form values when Name and MaaS secret are filled', async () => {
      const user = userEvent.setup();
      ...
      // Select a MaaS secret (required field)
      const selectSecretButton = screen.getByTestId('maas-secret-selector-select-secret');
      await user.click(selectSecretButton);

      await waitFor(() => {
        expect(nameInput).toHaveValue('Valid Name');
        expect(screen.getByTestId('maas-secret-selector-value')).toBeInTheDocument();
      });
    });
  ...
  describe('Add new connection', () => {
    it('should render the Add new connection button', () => {
      renderComponent();
      expect(screen.getByTestId('add-maas-connection-button')).toBeInTheDocument();
    });

    it('should open the MaasConnectionModal when button is clicked', async () => {
      const user = userEvent.setup();
      renderComponent();

      expect(screen.queryByTestId('maas-connection-modal')).not.toBeInTheDocument();

      await user.click(screen.getByTestId('add-maas-connection-button'));
    ...
    it('should close the modal when cancel is clicked', async () => {
      ...
      await user.click(screen.getByTestId('maas-modal-cancel'));
      expect(screen.queryByTestId('maas-connection-modal')).not.toBeInTheDocument();
    });
    ...
    it('should close modal and update form with new connection after submit', async () => {
      ...
      await user.click(screen.getByTestId('maas-modal-submit'));
      ...
      await waitFor(() => {
        expect(screen.getByTestId('maas-secret-selector-value')).toHaveTextContent(
          'new-secret-uuid',
        );
      });
    });
