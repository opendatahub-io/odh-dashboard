...
const BRANCH_PATTERN_ID = /^.+__pattern__branch-\d+$/;

const isBranchNode = (nodeId: string): boolean =>
  BRANCH_STEP_SUFFIX_ID.test(nodeId) ||
  BRANCH_STEP_PREFIX_ID.test(nodeId) ||
  BRANCH_PATTERN_ID.test(nodeId);

const getBranchIndex = (nodeId: string): number | undefined => {
  const patternMatch = /__pattern__branch-(\d+)$/.exec(nodeId);
  if (patternMatch?.[1]) {
    return parseBranchIndexFromSuffix(`branch-${patternMatch[1]}`);
  }
