...
func TestHTTPBFFClient_Call_Success(t *testing.T) {
	...
	client := NewHTTPBFFClient(server.URL+"/api/v1", BFFTargetModelRegistry, "test-auth-token", true, nil)
...
func TestHTTPBFFClient_Call_NoBody(t *testing.T) {
	...
	client := NewHTTPBFFClient(server.URL+"/api/v1", BFFTargetModelRegistry, "test-token", true, nil)
...
func TestHTTPBFFClient_Call_ErrorResponses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client := NewHTTPBFFClient(server.URL+"/api/v1", BFFTargetModelRegistry, "token", true, nil)
			...
			assert.Equal(t, BFFTargetModelRegistry, bffErr.Target)
		})
	}
}
...
func TestHTTPBFFClient_Call_ConnectionError(t *testing.T) {
	...
	client := NewHTTPBFFClient("http://localhost:1", BFFTargetModelRegistry, "token", true, nil)
...
func TestHTTPBFFClient_IsAvailable(t *testing.T) {
	t.Run("Available", func(t *testing.T) {
		...
		client := NewHTTPBFFClient(server.URL+"/api/v1", BFFTargetModelRegistry, "", true, nil)
	...
	t.Run("ConnectionRefused", func(t *testing.T) {
		client := NewHTTPBFFClient("http://localhost:1/api/v1", BFFTargetModelRegistry, "", true, nil)
		assert.False(t, client.IsAvailable(context.Background()))
	})
}

func TestHTTPBFFClient_GetBaseURL(t *testing.T) {
	client := NewHTTPBFFClient("http://test.svc:8080/api/v1", BFFTargetModelRegistry, "token", true, nil)
