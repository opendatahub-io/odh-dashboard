...
func TestNewDefaultBFFClientConfig(t *testing.T) {
	...
	assert.Equal(t, BFFTargetModelRegistry, mrConfig.Target)
	assert.Equal(t, "odh-dashboard", mrConfig.ServiceName)
	assert.Equal(t, 8043, mrConfig.Port)
	assert.Equal(t, "/api/v1", mrConfig.PathPrefix)
	assert.False(t, mrConfig.TLSEnabled)
	assert.Equal(t, "user_token", mrConfig.AuthMethod)
	assert.Equal(t, "x-forwarded-access-token", mrConfig.AuthTokenHeader)

	assert.Nil(t, config.GetServiceConfig(BFFTarget("unknown")))
}
