...
This document provides guidance for AI agents and developers working on the MLflow package.
The MLflow package is a modular-architecture frontend and Go backend-for-frontend (BFF) that
integrates with the ODH dashboard via Module Federation.
...
Describe every capability in `api/openapi/mlflow.yaml` (or a new file under `api/openapi/`). All
...
├── api/
│   └── openapi/
│       └── mlflow.yaml          # OpenAPI specification (contract-first)
...
| **federated**  | PatternFly | Micro-frontend in ODH dashboard        |
...
- PatternFly theme for ODH dashboard compatibility
...
- [OpenAPI Spec](api/openapi/mlflow.yaml) -
  [View in Swagger Editor](https://editor.swagger.io/?url=https://raw.githubusercontent.com/opendatahub-io/odh-dashboard/main/packages/mlflow/api/openapi/mlflow.yaml)
