[MLflow server set up]: ../../bff/docs/dev-guide.md

# Deploying the MLflow UI in a local cluster

For this guide, we will be using kind for locally deploying our cluster. See
the [MLflow server set up] guide for prerequisites on setting up kind
...
mlflow-ui-58755c4754-zdrnr    1/1     Running   0          11s
```

### 5. Access the MLflow UI running in the cluster
...
```shell
kubectl port-forward service/mlflow-ui-service 8080:8080 -n kubeflow
```
...
```shell
curl http://localhost:8080/api/v1/mlflow
```
