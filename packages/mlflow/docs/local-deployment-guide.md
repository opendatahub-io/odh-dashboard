...
## Local kubernetes deployment of MLflow

To test the BFF locally without mocking the k8s calls the MLflow backend can be deployed locally using kind.
...
#### 3. Deploy MLflow to cluster

You can now deploy the MR backend to your newly created cluster using the kustomize configs in the odh-dashboard repository by
running:

```shell
kubectl apply -k "https://github.com/opendatahub-io/odh-dashboard/manifests/kustomize/overlays/db"
```

Wait for the model registry deployment to spin up, alternatively run:

```shell
kubectl wait --for=condition=available -n kubeflow deployment/mlflow-deployment --timeout=1m
```
...
Two pods should be listed, `mlflow-db-xxx` and `mlflow-deployment-yyy` both should have a status of `Running`.
...
```shell
kubectl apply -k "https://github.com/opendatahub-io/odh-dashboard/manifests/kustomize/overlays/db?ref=kind"
```
...
```shell
kubectl port-forward svc/mlflow-service -n kubeflow 8080:8080
```
