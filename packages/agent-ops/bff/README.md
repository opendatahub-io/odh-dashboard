...
Agent endpoints validate `{ns}` and `{name}` as DNS-1123 identifiers. List discovery loads labeled Sandbox CRs (`agents.x-k8s.io/v1beta1/sandboxes`) matching either `opendatahub.io/agent-type=agent` (dashboard-deployed agents) or `openshell.ai/managed-by=openshell` (OpenShell-managed agents). Detail loads any authorized Sandbox CR by name (no label filter). List results are deduplicated by namespace and name. Per-request RBAC checks filter namespaces and gate detail access via `agents.x-k8s.io/sandboxes` list/get; stop/start require `patch`; delete requires `delete`. Deploy RBAC requires `agents.x-k8s.io/sandboxes` create and get. Optional `agentCard` enrichment adds external route URLs when available (best-effort; omitted when unavailable). `toolConnections` is intentionally empty until per-agent MCP linking is defined.
