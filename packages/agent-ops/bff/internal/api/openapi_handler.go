...
func NewOpenAPIHandler(logger *slog.Logger) (*OpenAPIHandler, error) {
	specPath := "openapi/src/agent-ops.yaml"
	specData, err := os.ReadFile(specPath)
	if err != nil {
		return nil, fmt.Errorf("reading OpenAPI spec %s: %w", specPath, err)
	}
...
func (h *OpenAPIHandler) HandleOpenAPIJSON(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	// Invariant: NewOpenAPIHandler ensures h.spec.Servers is non-empty.
	servers := make([]*openapi3.Server, 0, len(h.spec.Servers)+1)
	servers = append(servers, h.spec.Servers[0])
...
<head>
    ...
    <title>Agent Ops API Documentation</title>
