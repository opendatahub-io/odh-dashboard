...
describe('registryVersionToDeployData', () => {
  ...
  it('uses server_json._meta.image as the image', () => {
    const result = registryVersionToDeployData(mockServer(), mockVersion());
    expect(result.image).toBe('quay.io/mcp/weather:1.2.0');
  });

  it('falls back to an empty image when _meta.image is missing', () => {
  ...
  it('uses server_json._meta.configuration as the yaml', () => {
    const result = registryVersionToDeployData(mockServer(), mockVersion());
    expect(result.yaml).toBe('config:\n  port: 8080\n  path: /mcp\n');
  });

  it('falls back to an empty yaml when _meta.configuration is missing', () => {
    const result = registryVersionToDeployData(
      mockServer(),
      mockVersion({
        server_json: {
          name: 'io.github.example/weather-server',
          version: '1.2.0',
          _meta: { image: 'quay.io/mcp/weather:1.2.0' },
        },
      }),
    );
