...
The `dev-start-federated`, `dev-bff-federated`, and `dev-bff-e2e-cluster` targets need a `MAAS_API_URL` to connect to the MaaS API. If you don't set one, the Makefile will auto-detect it by querying your cluster's ingress domain:

```shell
oc get ingresses.config.openshift.io cluster -o jsonpath='{.spec.domain}'
```

The resulting URL is `https://maas.<CLUSTER_DOMAIN>/maas-api` (HTTPS is required; ingress redirects HTTP and breaks POST passthrough such as `api-keys/search`). For this to work you must be logged into the cluster (`oc login`).
