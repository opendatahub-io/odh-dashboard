...
func setupApiTest[T any](method, url string, body interface{}, k8Factory kubernetes.KubernetesClientFactory, identity *kubernetes.RequestIdentity) (T, *http.Response, error) {
	...
	repos, err := repositories.NewRepositories(logger, k8Factory, envConfig, subscriptionsRepo, policiesRepo, modelRefsRepo, externalProvidersRepo, externalModelsRepo, secretsRepo, yamlRepo)
	if err != nil {
		return empty, nil, err
	}
	app := &App{
