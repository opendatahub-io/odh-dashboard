...
var _ = Describe("SubscriptionHandlers", Ordered, func() {
	...
	var _ = Describe("GetSubscriptionFormDataHandler", Ordered, func() {
		...
		It("returns 200 with groups, model refs, and subscriptions", func() {
			actual, rs, err := setupApiTest[Envelope[*models.SubscriptionFormDataResponse, None]](
				http.MethodGet,
				"/api/v1/new-subscription",
				nil,
				k8Factory,
			...
			Expect(len(actual.Data.ModelRefs)).To(BeNumerically(">=", 2))
			var names []string
			...
			Expect(actual.Data.Subscriptions).NotTo(BeNil())
			Expect(len(actual.Data.Subscriptions)).To(BeNumerically(">", 0))
		})
	})
