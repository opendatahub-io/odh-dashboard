...
kustomize edit set image maas-ui=${IMG_UI_STANDALONE}
popd > /dev/null
...
kubectl wait --for=condition=available -n kubeflow deployment/maas-ui --timeout=1m
popd > /dev/null
...
kubectl port-forward svc/maas-ui-service -n kubeflow 8080:8080
