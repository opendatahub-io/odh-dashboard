...
describe('parsePipelineVersion', () => {
  it('should extract semver from pipeline version name', () => {
    expect(parsePipelineVersion('automl-3.5.0-ea.1')).toBe('3.5.0-ea.1');
  });

  it('should extract GA version without pre-release', () => {
    expect(parsePipelineVersion('automl-3.5.0')).toBe('3.5.0');
  });

  it('should handle version with ea.2 suffix', () => {
    expect(parsePipelineVersion('automl-3.6.0-ea.2')).toBe('3.6.0-ea.2');
  });
