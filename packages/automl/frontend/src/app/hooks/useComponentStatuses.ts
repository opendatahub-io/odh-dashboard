...
function componentIdToTaskId(componentId: string): string {
  return componentId.replace(/_/g, '-');
}

function getComponentsToFetch(
...
function mergeStatusIntoStageMap(
...
function isComponentFullyComplete(status: ComponentStatusFile): boolean {
