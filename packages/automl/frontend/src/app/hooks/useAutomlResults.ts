...
// return snake_case keys (e.g. "mean_absolute_error"). Normalize timeseries
// keys so downstream code only handles one form. See RHOAIENG-59989.
/* eslint-disable camelcase */
const TIMESERIES_METRIC_ALIASES: Record<string, string> = {
...
function normalizeTimeseriesMetricKeys(testData: Record<string, number>): Record<string, number> {
  const result: Record<string, number> = {};
  for (const [key, value] of Object.entries(testData)) {
    const normalized = TIMESERIES_METRIC_ALIASES[key.toUpperCase()] ?? key;
        ...
        queryFn: async ({ signal }) => {
          ...
          if (isRawTimeseriesModel(validated)) {
            model = {
              ...
              metrics: {
                // eslint-disable-next-line camelcase
                test_data: normalizeTimeseriesMetricKeys(validated.metrics.test_data),
              },
            };
          } else {
            model = {
