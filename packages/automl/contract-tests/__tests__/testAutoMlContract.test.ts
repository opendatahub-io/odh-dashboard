  ...
  describe('Pipeline Runs Endpoints', () => {
    describe('List Pipeline Runs', () => {
      ...
      it('should return 400 when namespace parameter is missing', async () => {
        const result = await apiClient.get('/api/v1/pipeline-runs');
      ...
      it('should return 400 for empty namespace', async () => {
        const result = await apiClient.get('/api/v1/pipeline-runs?namespace=');
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(400);
        }
    ...
    describe('Get Single Pipeline Run', () => {
      ...
      it('should return 400 when namespace parameter is missing', async () => {
        const result = await apiClient.get('/api/v1/pipeline-runs/run-abc123-def456');
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(400);
        }
      ...
      it('should return 400 for empty namespace', async () => {
        const result = await apiClient.get('/api/v1/pipeline-runs/run-abc123-def456?namespace=');
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.status).toBe(400);
        }
    ...
    // NOTE: POST /api/v1/pipeline-runs tests are intentionally excluded
    // until PR #6625 is merged, which adds CreatePipelineRun functionality.
    // Once merged, add tests for:
    // - Creating pipeline runs with required fields
    // - Creating pipeline runs with all optional fields
    // - Validation of missing required fields (400)
    // - Validation of invalid parameters (400)
    // - Rejection of unknown JSON fields (400)
  });
});
