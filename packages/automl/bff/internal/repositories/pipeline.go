...
const (
	...
	// marks them as AutoML managed pipelines. This would be more robust than name matching.
	// Example: pipeline.metadata.labels["odh.io/managed-pipeline"] = "automl"
	// or pipeline.spec.pipelineType = "automl"
...
// DiscoverAutoMLPipeline finds the AutoML managed pipeline in the given namespace.
//
// This method automatically discovers the AutoML pipeline by:
...
func (r *PipelineRepository) DiscoverAutoMLPipeline(
	...
	// Find AutoML pipeline by name (case-insensitive prefix match)
	// TODO: Replace with attribute-based lookup once managed pipeline metadata is available
	var automlPipeline *models.KFPipeline
	for i := range pipelinesResp.Pipelines {
		...
		if strings.HasPrefix(strings.ToLower(pipeline.DisplayName), strings.ToLower(pipelineNamePrefix)) {
			automlPipeline = pipeline
			break
		}
	}

	if automlPipeline == nil {
		return nil, fmt.Errorf("AutoML pipeline not found in namespace %s (searched for pipelines starting with %q)",
			namespace, pipelineNamePrefix)
	}

	// Get pipeline versions for the AutoML pipeline
	versionsResp, err := client.ListPipelineVersions(ctx, automlPipeline.PipelineID)
	if err != nil {
		return nil, fmt.Errorf("failed to list versions for pipeline %s: %w", automlPipeline.PipelineID, err)
	}
	...
	discovered := &DiscoveredPipeline{
		PipelineID:        automlPipeline.PipelineID,
		PipelineVersionID: version.PipelineVersionID,
		PipelineName:      automlPipeline.DisplayName,
		Namespace:         namespace,
