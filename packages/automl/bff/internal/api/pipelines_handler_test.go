...
func TestCreatePipelineRunHandler(t *testing.T) {
	...
	validBody := `{"display_name":"new-run","train_data_secret_name":"s","train_data_bucket_name":"b","train_data_file_key":"f"}`
	...
	tests := []struct {
		...
		},
		{
			...
			body:           validBody,
			repoResult:     nil,
			repoErr:        fmt.Errorf("missing label_column: %w", repositories.ErrValidation),
			wantStatusCode: http.StatusBadRequest,
			wantBodySubstr: `"code": "400"`,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.body == validBody {
				repo.On("CreateRun", mock.Anything, ns, mock.AnythingOfType("models.CreateAutoMLRunRequest")).
					Return(tt.repoResult, tt.repoErr)
			}
...
func TestCreatePipelineRunHandler_TimeSeriesID(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "empty ID for two columns", id: str(""), columns: []string{"date", "sales"}, wantStatus: 200},
		{name: "whitespace ID for two columns", id: str(" "), columns: []string{"date", "sales"}, wantStatus: 200},
		...
		{name: "three columns empty ID", id: str(""), columns: []string{"date", "sales", "store"}, wantStatus: 400},
		{name: "three columns blank ID", id: str("\ufeff  "), columns: []string{"date", "sales", "store"}, wantStatus: 400},
