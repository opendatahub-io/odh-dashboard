...
func TestMiddleware_RequireAccessToService(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantBodySubstr: "pipeline servers",
			expectMockCall: true,
		},
		{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectMockCall {
				k8sMock.On("CanAccessResource",
					// context.Context is matched by mock.AnythingOfType or mock.Anything
					// Using mock.Anything for context since it changes per request
					mock.Anything,
					tt.namespace,
					"list",
