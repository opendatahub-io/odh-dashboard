...
func TestGetSecretsHandler_DoesNotLogCredentials(t *testing.T) {
	...
	t.Run("response body sanitization", func(t *testing.T) {
		...
		// aws-secret: 4 fields (4 redacted: access_key, secret_key, region, endpoint)
		// generic-secret: 2 fields (2 redacted: password, username)
		// Total: 6 redacted values expected (aws_s3_bucket is allowed, but not in this test data)
		// Note: endpoint and region are also redacted
		assert.GreaterOrEqual(t, redactedCount, 6, "Should have at least 6 [REDACTED] values in response")
	})
}
