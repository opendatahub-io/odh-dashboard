...
// namespace in context and optional query params. The queryString is set
// directly on the URL to avoid issues with special characters in httptest.NewRequest.
...
func TestGetS3FileHandler(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "unsupported view returns 400",
			namespace:        "test-ns",
			key:              "data.csv",
			queryString:      "secretName=my-secret&view=unsupported",
			setupMock:        func(repo *mockS3Repo) {},
			wantStatusCode:   http.StatusBadRequest,
			wantBodyContains: "unsupported view",
		},
		{
		...
		{
			name:        "view=schema success returns column schema",
			namespace:   "test-ns",
			key:         "data.csv",
			queryString: "secretName=my-secret&view=schema",
			setupMock: func(repo *mockS3Repo) {
				repo.On("GetCSVSchema", mock.Anything, mock.Anything, "data.csv").
					Return(helper.CSVSchemaResult{
						Columns: []helper.ColumnSchema{
							{Name: "age", Type: "integer", TaskType: "regression"},
						},
						ParseWarnings: 0,
					}, nil)
			},
			wantStatusCode:   http.StatusOK,
			wantBodyContains: "age",
		},
		{
			name:        "view=schema repo error returns appropriate error",
			namespace:   "test-ns",
			key:         "bad.csv",
			queryString: "secretName=my-secret&view=schema",
			setupMock: func(repo *mockS3Repo) {
				repo.On("GetCSVSchema", mock.Anything, mock.Anything, "bad.csv").
					Return(helper.CSVSchemaResult{}, fmt.Errorf("csv issue: %w", helper.ErrCSVValidation))
			},
			wantStatusCode:   http.StatusBadRequest,
			wantBodyContains: "csv issue",
		},
		{
...
// Query parameters are set on the URL directly to avoid encoding issues.
...
func TestPostS3FileHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(repo *mockS3Repo) {
				repo.On("UploadCSVFile", mock.Anything, mock.Anything, "upload.csv", mock.Anything, "text/csv", "upload.csv", 0).
		...
		{
			name:            "repo ErrCSVUploadValidation returns 400",
			namespace:       "test-ns",
			key:             "upload.csv",
			queryString:     "secretName=my-secret",
			filename:        "upload.csv",
			...
			setupMock: func(repo *mockS3Repo) {
				repo.On("UploadCSVFile", mock.Anything, mock.Anything, "upload.csv", mock.Anything, "text/csv", "upload.csv", 0).
					Return("", fmt.Errorf("invalid CSV: %w", repositories.ErrCSVUploadValidation))
			},
			wantStatusCode: http.StatusBadRequest,
		},
		{
...
func TestHandleS3RepoError(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "repositories ErrCSVUploadValidation",
			err:            fmt.Errorf("wrapped: %w", repositories.ErrCSVUploadValidation),
			key:            "",
			wantStatusCode: http.StatusBadRequest,
		},
		{
			name:           "helper ErrCSVValidation",
			err:            fmt.Errorf("wrapped: %w", helper.ErrCSVValidation),
			key:            "",
			wantStatusCode: http.StatusBadRequest,
		},
		{
...
// --- validateGetS3FilesParams tests ---

func TestValidateGetS3FilesParams(t *testing.T) {
	tests := []struct {
		name    string
		query   string
		wantErr string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := validateGetS3FilesParams(req.URL.Query())
			if tt.wantErr != "" {
				assert.Error(t, err)
