...
1. **`GetS3Credentials()`** in `s3.go` (line 90-95)
...
2. **`GetS3Credentials()`** in `s3_mock.go` (line 77-79)
...
All tests are located in `internal/api/s3_handler_test.go` (lines 638-863).
