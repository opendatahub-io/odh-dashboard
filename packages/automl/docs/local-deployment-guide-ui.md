[AutoML server set up]: local-deployment-guide.md

# Deploying the AutoML UI in a local cluster

For this guide, we will be using kind for locally deploying our cluster. See
the [AutoML server set up] guide for prerequisites on setting up kind
...
### 5. Access the AutoML UI running in the cluster
