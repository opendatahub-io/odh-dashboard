...
- A two-column dataset needs a timestamp and a numeric target. A single-item time series needs no manual item ID column. For multiple items, supply an item ID column. Synthetic ID injection is owned by the pipeline (RHOAIENG-81174).
