...
- A two-column dataset needs a timestamp and a numeric target. A single-item time series needs no manual item ID column. For two-column datasets, the ID dropdown is disabled and displays "Not required". For datasets with three or more columns, the UI requires an ID selection. Synthetic ID injection is owned by the pipeline (RHOAIENG-81174).
