...
const createMockConfig = (
  ...
  ({
    ...
    metadata: {
      ...
      labels: { 'opendatahub.io/config-type': 'workload-single-node' },
    },
    spec: {},
...
describe('TopologyConfigurationsTable', () => {
  ...
  it('should keep toggle disabled until watch data updates after successful patch', async () => {
    ...
    const { rerender } = render(<TopologyConfigurationsTable configs={[enabledConfig]} />);

    const toggle = screen.getByTestId('topology-config-enabled-toggle');
    ...
    rerender(<TopologyConfigurationsTable configs={[disabledConfig]} />);

    const updatedToggle = screen.getByTestId('topology-config-enabled-toggle');
  ...
  it('should re-enable toggle immediately on patch error', async () => {
    ...
    const enabledConfig = createMockConfig('my-config', true);
    render(<TopologyConfigurationsTable configs={[enabledConfig]} />);
