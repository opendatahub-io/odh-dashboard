...
describe('isK8sNameDescriptionDataValid', () => {
  ...
  it('should be false when immutable but invalidCharacters even if routeNameTooLong', () => {
    expect(
      isK8sNameDescriptionDataValid(
        mockK8sNameDescriptionFieldData({
          name: 'test',
...
describe('handleUpdateLogic with namespace (route name validation)', () => {
  ...
  it('should treat options.namespace undefined as no namespace (not stale state)', () => {
    const defaults = setupDefaults({
      limitNameResourceType: LimitNameResourceType.WORKBENCH,
      namespace: 'a-very-long-project-namespace-name',
      safePrefix: 'wb-',
    });
    // State has long namespace; override with undefined (async not resolved) so
    // route validation uses undefined — not the stale state namespace.
    const result = handleUpdateLogic(defaults, { namespace: undefined })(
      'name',
      'this is a workbench with a long name',
    );
    expect(result.k8sName.state.routeNameTooLong).toBe(false);
