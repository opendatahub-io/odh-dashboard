...
func TestPrepareResponseParams_ReasoningEffort(t *testing.T) {
	client := &LlamaStackClient{}

	t.Run("should set Reasoning when ReasoningEffort is provided", func(t *testing.T) {
		params := CreateResponseParams{
			Input:           InputUnion{Text: "Think about this"},
		...
		assert.Equal(t, "medium", string(result.Reasoning.Effort))
	})

	t.Run("should leave Reasoning zero-value when ReasoningEffort is empty", func(t *testing.T) {
		params := CreateResponseParams{
			Input: InputUnion{Text: "Normal query"},
		...
		assert.Equal(t, "", string(result.Reasoning.Effort))
	})

	t.Run("should pass through various effort levels", func(t *testing.T) {
		efforts := []string{"minimal", "low", "medium", "high"}
		for _, effort := range efforts {
			t.Run("effort="+effort, func(t *testing.T) {
				params := CreateResponseParams{
					Input:           InputUnion{Text: "test"},
					Model:           "test-model",
				...
				result, err := client.prepareResponseParams(params)
				require.NoError(t, err)
				assert.Equal(t, effort, string(result.Reasoning.Effort))
			})
		}
	})
}
