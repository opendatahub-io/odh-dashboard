...
export const useMCPToolSelections = (): {
   ...
   */
  const getToolSelections = (namespace: string, serverUrl: string): string[] | undefined =>
   ...
   */
  const saveToolSelections = (
    namespace: string,
    serverUrl: string,
    toolNames: string[] | undefined,
  ): void => {
    if (toolNames === undefined) {
      // Remove the entry for this server to indicate all tools are allowed
