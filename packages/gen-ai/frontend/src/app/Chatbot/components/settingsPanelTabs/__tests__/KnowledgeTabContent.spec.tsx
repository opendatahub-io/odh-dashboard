...
import { useChatbotConfigStore, MODEL_1_CONFIG_ID } from '~/app/Chatbot/store';
...
describe('KnowledgeTabContent', () => {
  ...
  it('renders RAG toggle as checked when isRagEnabled is true', () => {
    ...
    useChatbotConfigStore.getState().updateRagEnabled(MODEL_1_CONFIG_ID, true);
  ...
  it('calls updateRagEnabled and fires tracking event when RAG toggle is changed', async () => {
    ...
    expect(useChatbotConfigStore.getState().getConfiguration(MODEL_1_CONFIG_ID)?.isRagEnabled).toBe(
      true,
    );
