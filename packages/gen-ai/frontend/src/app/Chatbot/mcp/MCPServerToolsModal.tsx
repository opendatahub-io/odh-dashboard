...
const MCPServerToolsModal: React.FC<MCPServerToolsModalProps> = ({
  ...
  // Fetch tools using the hook - only when modal is open
  const {
    tools: apiTools,
    toolsLoaded,
    toolsLoadError,
  ...
  // Convert API tools to UI format and initialize local state
  const tools = React.useMemo(
    () =>
      apiTools.map((apiTool, index) => ({
        ...
        permissions: [], // Not provided by API, keeping for compatibility
