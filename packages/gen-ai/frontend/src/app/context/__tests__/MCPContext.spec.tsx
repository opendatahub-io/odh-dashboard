...
// Mock dependencies
jest.mock('~/app/context/ProjectContext');
...
describe('MCPContext', () => {
  ...
  beforeEach(() => {
    ...
    mockUseProject.mockReturnValue({
      ...
      availableProjects: ['test-namespace', 'other-namespace'],
      isLoading: false,
      error: null,
    });
  ...
  describe('MCPContextProvider', () => {
    ...
    it('uses browser storage with correct key and options', () => {
      ...
      expect(mockUseBrowserStorage).toHaveBeenCalledWith(
        'gen-ai-playground-servers-test-namespace',
        [],
        true, // jsonify
    ...
    it('updates current namespace when project changes', async () => {
      const wrapper: React.FunctionComponent<{ children: React.ReactNode }> = ({ children }) => (
        <MCPContextProvider>{children}</MCPContextProvider>
      );

      const { result, rerender } = renderHook(() => useMCPContext(), { wrapper });

      expect(result.current.currentNamespace).toBe('test-namespace');

      // Change project
      mockUseProject.mockReturnValue({
        selectedProject: 'new-namespace',
        setSelectedProject: jest.fn(),
        availableProjects: ['new-namespace', 'other-namespace'],
        isLoading: false,
        error: null,
      });
      ...
      await waitFor(() => {
        expect(result.current.currentNamespace).toBe('new-namespace');
      });
    });
  });
