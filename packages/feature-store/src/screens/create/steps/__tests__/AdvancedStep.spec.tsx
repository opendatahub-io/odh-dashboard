...
describe('AdvancedStep', () => {
  ...
  describe('Cron job', () => {
    ...
    it('clears cronJob when schedule is emptied', () => {
      const { setData } = renderStep({ cronJob: { schedule: '*/5 * * * *' } });
      fireEvent.change(screen.getByPlaceholderText('0 */6 * * *'), { target: { value: '' } });
      expect(setData).toHaveBeenCalledWith('cronJob', undefined);
    });

    it('clears cronJob when non-schedule fields are set without a schedule', () => {
      const { setData } = renderStep();
