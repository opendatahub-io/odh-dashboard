...
describe('DeploymentProgressPage', () => {
  ...
  ])('shows %s phase in status card', (phase) => {
    const useWatchMock = jest.requireMock('../../../hooks/useWatchFeatureStoreDeployment').default;
    useWatchMock.mockReturnValue({ ...mockDeploymentStatus, phase });
    renderPage();
    expect(screen.getByTestId('deployment-status-card')).toHaveTextContent(phase);
  });
  ...
  it('renders conditions card with semantic labels, messages, and reasons', () => {
    const useWatchMock = jest.requireMock('../../../hooks/useWatchFeatureStoreDeployment').default;
    useWatchMock.mockReturnValue({
      ...mockDeploymentStatus,
      conditions: [
        { type: 'Ready', status: 'True', message: 'All pods running' },
    ...
    expect(card).toHaveTextContent('All pods running');
    expect(card).toHaveTextContent('Available');
    expect(card).toHaveTextContent('MinimumReplicasUnavailable');
    expect(card).toHaveTextContent('Progressing');
    expect(card).toHaveTextContent('Pending');
  });

  it.each([
    ['Running', 'feast-pod-abc'],
    ['Failed', 'feast-pod-fail'],
    [undefined, 'no-status-pod'],
  ] as [string | undefined, string][])('renders pod with phase=%s', (phase, podName) => {
    const useWatchMock = jest.requireMock('../../../hooks/useWatchFeatureStoreDeployment').default;
    useWatchMock.mockReturnValue({
      ...mockDeploymentStatus,
    ...
    renderPage();
    expect(screen.getByTestId('deployment-pods-card')).toBeInTheDocument();
  ...
  it('renders safely when conditions and pods are malformed (non-array)', () => {
    const useWatchMock = jest.requireMock('../../../hooks/useWatchFeatureStoreDeployment').default;
    useWatchMock.mockReturnValue({
      ...mockDeploymentStatus,
