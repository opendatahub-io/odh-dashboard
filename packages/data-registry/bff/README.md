...
- **Relaying**: request method, body, and the raw query string are forwarded unchanged (query
  ...
  `Authorization` (always rebuilt, see above) and `X-User`/`kubeflow-userid`/`kubeflow-groups`
  (always stripped) — the upstream contract trusts those identity headers for attribution (e.g.
  the `registered_by` field), so a caller-supplied value is never relayed as-is. The upstream
  response status, body, and headers are relayed back verbatim by `httputil.ReverseProxy` —
  including error responses, so Iceberg REST error shapes (`{"error": {"message", "type", "code"}}`)
  reach the frontend untransformed.
- **Configuration**: the upstream base URL comes from `-data-registry-api-url`/`DATA_REGISTRY_API_URL`
  when set (local dev/tests), otherwise from a ConfigMap in the pod's own namespace at startup
  (name/key configurable, see flags table above). If neither is available — e.g. the Data Registry
  backend from [RHAISTRAT-2381](https://redhat.atlassian.net/browse/RHAISTRAT-2381) hasn't been
  deployed yet — the BFF logs a warning at startup and every `/api/v1/*` proxy route returns `503
  Service Unavailable` until it's configured, rather than failing to start.
