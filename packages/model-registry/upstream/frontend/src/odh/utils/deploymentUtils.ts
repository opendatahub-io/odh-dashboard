...
export const useModelDeploymentDetection = (): UseModelDeploymentDetectionResult => {
  const { deployments, loaded } = useDeploymentsState();

  const hasRegisteredModelDeploymentByVersionIds = React.useCallback(
    (mvIds: string[]): DeploymentDetectionResult => {
  ...
  const hasModelVersionDeployment = React.useCallback(
    (mvId: string): DeploymentDetectionResult => hasRegisteredModelDeploymentByVersionIds([mvId]),
    [hasRegisteredModelDeploymentByVersionIds],
  );

  const hasRegisteredModelDeployment = React.useCallback(
    (rmId: string, modelVersions: ModelVersion[]): DeploymentDetectionResult => {
      if (!loaded || !deployments) {
        return { hasDeployment: true, loaded };
      }
