...
const ensureMlflowCR = (namespace: string): Cypress.Chainable<CommandLineResult> => {
  const checkCommand = `oc get mlflows.mlflow.opendatahub.io -n ${namespace} --no-headers 2>/dev/null | head -1`;
  return cy.exec(checkCommand, { failOnNonZeroExit: false }).then((result) => {
    ...
    return cy
      .exec(`oc apply -n ${namespace} -f cypress/fixtures/e2e/promptManagement/mlflowCR.yaml`, {
...
export const deleteMlflowCR = (namespace: string): Cypress.Chainable<CommandLineResult> =>
  cy
    .exec(`oc delete mlflows.mlflow.opendatahub.io --all -n ${namespace}`, {
      failOnNonZeroExit: false,
    })
    .then((result) => {
      if (result.code !== 0) {
...
const waitForMlflowCRReady = (namespace: string): Cypress.Chainable<Cypress.Exec> =>
  pollUntilSuccess(
    `oc get mlflows.mlflow.opendatahub.io -n ${namespace} -o json | jq -e '.items[0].status.address.url'`,
