 ...
 * On confirmed failure, throws a meaningful error to fail fast instead of waiting
 * for the full readiness timeout.
 *
 * @param modelName The display name of the deployed model
 */
export const assertModelDeploymentHealthy = (modelName: string): void => {
  cy.get('body').then(($body) => {
    ...
    if (!hasFailed) {
      cy.log(`No "Failed" status detected for ${modelName} — proceeding`);
      return;
    }

    cy.log(`"Failed" status detected for ${modelName} — waiting 10s and re-checking...`);
    cy.wait(10000);
    ...
    cy.get('body').then(($bodyRetry) => {
      ...
      if (!stillFailed) {
        cy.log(`"Failed" status resolved after retry for ${modelName} — proceeding`);
        return;
      }

      throw new Error(
        `Model deployment "${modelName}" entered Failed state. Failing fast to avoid waiting for the full readiness timeout.`,
      );
    });
  });
};
