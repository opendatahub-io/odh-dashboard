import type { DataScienceProjectData, DashboardConfig } from '../../../../types';
...
describe('Verify Cluster Storage - Creating, Editing and Deleting', () => {
  ...
  retryableBefore(() => {
    ...
    cy.getDashboardConfig().then((config) => {
      ...
      if (!dashboardConfig.notebookController.pvcSize) {
        ...
        cy.log('Using default PVC size: 20Gi');
      }
  ...
  it(
    ...
    {
      tags: ['@Sanity', '@SanitySet1', '@ODS-1824', '@Dashboard', '@ci-dashboard-regression-tags'],
    },
    () => {
