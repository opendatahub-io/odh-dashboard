...
describe('Global Project Settings', () => {
  beforeEach(() => {
    ...
    cy.interceptK8sList(
      { model: ProjectModel },
      mockK8sResourceList([
        mockProjectK8sResource({ name: 'project-alpha', displayName: 'Project Alpha' }),
        mockProjectK8sResource({ name: 'mlflow-workspace', displayName: 'MLflow Workspace' }),
      ]),
    );
  });
  ...
  it('should display the currently configured namespace', () => {
    cy.interceptOdh(
      'GET /api/config',
      mockDashboardConfig({ globalMLflowNamespaces: ['mlflow-workspace'] }),
    );
    cy.interceptOdh(
      'GET /api/cluster-settings',
  ...
  it('should enable page-level save and submit when a project is selected', () => {
    ...
    cy.findByTestId('submit-cluster-settings').should('be.enabled');
    cy.findByTestId('submit-cluster-settings').click();
