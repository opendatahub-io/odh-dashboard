...
describe('Feature Store Manage Page', () => {
  describe('with feature stores', () => {
    ...
    it('should display the manage page with a table of feature stores', () => {
      ...
      featureStoreManagePage.findRowByName('store-alpha').should('exist');
      featureStoreManagePage.findRowByName('store-beta').should('exist');
    });
    ...
    it('should display status labels for each feature store', () => {
      featureStoreManagePage.visit();
      featureStoreManagePage.findRowByName('store-alpha').should('contain.text', 'Ready');
      featureStoreManagePage.findRowByName('store-beta').should('contain.text', 'Installing');
    });

    it('should display version for each feature store', () => {
      featureStoreManagePage.visit();
      featureStoreManagePage.findRowByName('store-alpha').should('contain.text', '0.41.0');
      featureStoreManagePage.findRowByName('store-beta').should('contain.text', '0.40.0');
    });

    it('should display the Primary label on UI-labeled feature stores', () => {
      featureStoreManagePage.visit();
      featureStoreManagePage.findRowByName('store-alpha').should('contain.text', 'Primary');
      featureStoreManagePage.findRowByName('store-beta').should('not.contain.text', 'Primary');
    });
    ...
    it('should display namespace for each feature store', () => {
      featureStoreManagePage.visit();
      featureStoreManagePage.findRowByName('store-alpha').should('contain.text', NAMESPACE);
    });
  ...
  describe('delete flow', () => {
    ...
    it('should open the delete modal from the kebab menu', () => {
      featureStoreManagePage.visit();
      featureStoreManagePage.findKebabAction('deletable-store', 'Delete');
    ...
    it('should disable the delete button until the store name is confirmed', () => {
      featureStoreManagePage.visit();
      featureStoreManagePage.findKebabAction('deletable-store', 'Delete').click();
      deleteFeatureStoreModal.shouldBeOpen('deletable-store');
