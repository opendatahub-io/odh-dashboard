# Installing mod-arch-starter via `pnpm dlx`
...
```bash
pnpm dlx mod-arch-installer my-new-module
```
...
| `--skip-install` | Skips running `pnpm install` inside `frontend/`. | `false` |
...
pnpm dlx mod-arch-installer mr-ui --flavor kubeflow

# PatternFly-only starter that omits mod-arch-kubeflow entirely
pnpm dlx mod-arch-installer experiments-ui --flavor default
```
...
2. Run the Go BFF locally: `cd <your-project>/bff && make run` (requires Go 1.26+).
